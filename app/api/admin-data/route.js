...
export async function GET(request) {
  ...
  if (!SUPABASE_URL || !SUPABASE_ANON_KEY) {
    return NextResponse.json({ error: 'SUPABASE vars no configuradas' }, { status: 500 });
  }
...
async function fetchStripe() {
  ...
  const productsWithPrices = products.map(p => ({
    ...p,
    price: p.default_price && pricesMap[p.default_price] ? pricesMap[p.default_price] : null
  }));
...
async function fetchBrevoEmailStats() {
  ...
  return {
    sent, delivered, opens, clicks, unsubscribes,
...
async function fetchAcademia() {
  ...
  const [profilesRes, enrollmentsRes, lessonsRes, quizRes, profilesDataRes] = await Promise.all([
    fetch(`${url}/rest/v1/profiles?select=id&limit=0`, { headers: { ...headers, 'Prefer': 'count=exact' } }),
    fetch(`${url}/rest/v1/enrollments?select=id&limit=0`, { headers: { ...headers, 'Prefer': 'count=exact' } }),
    fetch(`${url}/rest/v1/lesson_progress?select=id&limit=0`, { headers: { ...headers, 'Prefer': 'count=exact' } }),
...
async function fetchEmailFunnel() {
  ...
  const [totalRes, pendingRes, sentRes] = await Promise.all([
    ...
    fetch(`${url}/rest/v1/email_sequence?select=id&sent=eq.false&limit=0`, { headers: { ...headers, 'Prefer': 'count=exact' } }),
    fetch(`${url}/rest/v1/email_sequence?select=id&sent=eq.true&limit=0`, { headers: { ...headers, 'Prefer': 'count=exact' } })
  ]);
...
async function fetchNewFeatures() {
  ...
  const [compatRes, weeklyRes, emailQueueRes, postPurchaseRes, productBreakdownRes] = await Promise.all([
    fetch(`${url}/rest/v1/funnel_events?select=id&event_type=eq.compatibilidad&limit=0`, { headers: { ...headers, 'Prefer': 'count=exact' } }),
    fetch(`${url}/rest/v1/funnel_events?select=id&event_type=eq.weekly_horoscope&limit=0`, { headers: { ...headers, 'Prefer': 'count=exact' } }),
    fetch(`${url}/rest/v1/email_sequence?select=id&sent=eq.false&limit=0`, { headers: { ...headers, 'Prefer': 'count=exact' } }),
    fetch(`${url}/rest/v1/email_sequence?select=id&step=gte.10&sent=eq.true&limit=0`, { headers: { ...headers, 'Prefer': 'count=exact' } }),
